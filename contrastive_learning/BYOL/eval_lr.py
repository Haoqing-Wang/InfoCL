...
def train(loader, online_model, model, criterion, optimizer):
    loss_epoch = 0
    model.train()
    for step, (x, y) in enumerate(loader):
        ...
        with torch.no_grad():
            if args.model == 'LBE':
                _, _, h, _, _, _, _, _ = online_model(x.cuda())
            else:  # Orig/RC
                h, _, _ = online_model(x.cuda())
        output = model(h)
...
def test(loader, online_model, model):
    right_num = 0
    all_num = 0
    model.eval()
    with torch.no_grad():
        for x, y in loader:
            x, y = x.cuda(), y.cuda()
            if args.model == 'LBE':
                _, _, h, _, _, _, _, _ = online_model(x)
            else:  # Orig/RC
                h, _, _ = online_model(x)
            output = model(h)
...
def main():
    ...
    online_model, _, _, _, _ = load_model(args, reload_model=True, load_path=epoch_dir, data=data)
    online_model = online_model.cuda()
    online_model.eval()

    # Logistic Regression
    n_classes = get_data_nclass(args.testset)
    model = LogisticRegression(online_model.n_features, n_classes).cuda()
    ...
    for epoch in range(args.logistic_epochs):
        loss_epoch = train(train_loader, online_model, model, criterion, optimizer)
        ...
        # final testing
        test_current_acc = test(test_loader, online_model, model)
